...
from training.shmem_vec_env import ShmemVecEnv
from baselines.common.vec_env.vec_normalize import \
    VecNormalize as VecNormalize_

from training.sonic_wrapper import SonicSafetyWrapper
...
class TerminalDummyVecEnv(DummyVecEnv):
    """DummyVecEnv variant that preserves the final observation before auto-reset."""
    ...
    def step_wait(self):
        for e in range(self.num_envs):
            ...
            if self.buf_dones[e]:
                self.buf_infos[e] = dict(self.buf_infos[e])
                self.buf_infos[e]["terminal_observation"] = obs
...
def make_env(env_id, seed, rank, log_dir, allow_early_resets, config=None, envNum=1, ax=None, test_case=-1):
    def _thunk():
        ...
        if envNum > 1:
            ...
            env.phase = 'test'
        # env.phase = 'train'  # <--- 强制所有环境都用 train 模式
        # print(f"【DEBUG】已强制设置 env.phase = {env.phase}")
        ...
        if getattr(config.training, 'use_sonic_wrapper', True):
            env = SonicSafetyWrapper(env, config)
