...
class Config(object):
    ...
    training.aux_per_human_risk_coef = 0.03
    training.aux_action_risk_coef = 0.00
    training.future_human_risk_K = 4
    training.future_human_risk_gamma = 0.90
    training.human_risk_safe_margin = 0.10
    training.human_risk_use_aci_feature = True
    training.human_risk_aci_scale = 0.20
    training.human_risk_pos_thresh = 0.03
    training.human_risk_pos_weight = 4.0
    ...
    class safety(object):
        ...
        jc_ema_beta = 0.90
        violation_deadband = 0.05
        lambda_kp = 0.04
        lambda_ki = 0.004
        integral_min = -50.0
        integral_max = 50.0
        lambda_output_tau = 0.94
...
def apply_experiment_variant(config, for_training=False):
    ...
    elif variant == 'wo_vs_shaping':
        tr.use_danger_aux = False
        tr.use_per_human_risk_aux = False
        tr.use_volatility_aci = False
        tr.use_vs_obs = False
        tr.use_aci_in_cost = False
        suffix = '_wo_vs_shaping'

    else:
        raise ValueError(
            f"Unknown training.exp_variant={variant}. "
            f"Use one of: full, wo_volatility_aci, wo_danger_aux, wo_vs_shaping"
        )
...
Config.training.model_name = 'height'
...
Config.training.output_dir = 'trained_models/baselines/unified/height'
